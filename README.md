...
**Plugin ID:** `display.workspaces` · **Version:** 2.5.0
...
No extra Python packages, automatic layout learning, individual-window placement tracking, rotation editor, or HDR control panel. Manual previews on new connectors use an on-demand, session-lived layout owner; no startup service is installed.
...
- **New connectors can be arranged manually without adding monitor rules.** Native geometry-only output management preserves unrelated display policy; a transient session owner retains Apply's result. Automatic display changes still require supported exact connector rules. Dynamic, broad, conditional, or ambiguous declarations remain unsupported.
...
- `output_management.py`, `output_owner.py`: geometry-only native transport and event-driven session ownership for new connectors.
