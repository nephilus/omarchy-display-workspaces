...
**See which workspaces belong to which display and arrange screens without editing configuration from one compact Omarchy bar panel.**

**Plugin ID:** `display.workspaces` · **Version:** 2.7.0
...
Real plugin UI with **synthetic example displays/workspaces**, captured on an opaque, neutral background—not a personal desktop. Normal styling and translucency follow your Omarchy theme.
...
Choose an advertised resolution/refresh rate and valid scale, then drag enabled displays on the map or use coordinates and relative-placement controls. **Preview (20s)** tries the complete draft live; **Apply** keeps it for this session. The plugin deliberately does not enable or disable outputs.
...
- **Separate presentation from live changes:** names/icons/order save immediately; layout and workspace moves use a session-only preview. Saving a profile is separate.
...
No extra Python packages, automatic layout learning, individual-window placement tracking, output enable/disable controls, rotation editor, or HDR control panel. Native output management handles manual geometry previews on new connectors through the on-demand layout owner and independent rollback worker; no startup service is installed.
After system resume, the elected widget releases stale native geometry ownership, refreshes monitors/workspaces, and waits for four stable healthy observations before unblocking layout automation. If outputs remain invalid or the refresh cannot complete within ten seconds, automation stays blocked until a real monitor connection change retries recovery.
...
## Safety and limits
- **Output topology is outside the plugin:** Displays can be arranged only while enabled and healthy. Enable, disable, undock, or redock outputs with Hyprland configuration or a dedicated display tool before opening this panel.
...
- **New connectors can be arranged manually without adding monitor rules.** Native output management preserves unrelated display policy while applying geometry changes. A transient session owner journals recovery and retains native geometry overrides. Automatic display changes still require supported exact connector rules. Dynamic, broad, conditional, or ambiguous declarations remain unsupported.
...
- `output_management.py`, `output_owner.py`: native geometry transport, session ownership, and recovery journaling.
