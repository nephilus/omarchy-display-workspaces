...
**Plugin ID:** `display.workspaces` · **Version:** 2.2.1
...
![Workspaces panel with editable display names, icon selectors, group-order controls, and workspace cards](docs/images/workspaces.png)
...
### Recall a familiar setup manually

![Profiles panel with a named live arrangement, hardware-match status, and Save, Replace, Delete, Load, and Cancel footer actions](docs/images/profiles.png)

**Save current** remembers the live display geometry and workspace placement—not untested draft edits. Select a profile and **Load into draft**, review it, then Preview and Apply. Hardware-first matching supports connector changes; nothing switches automatically. Replace and Delete require confirmation and preserve backups.
...
A flat workspace list hides which screen owns what; simple layout changes should not require a full settings application or hand-editing coordinates. This plugin combines a per-display overview with three compact native tabs, manual profiles, and deliberate confirmation.
...
No extra Python packages, persistent background service, automatic profile switching, rotation editor, or HDR control panel.
...
- **Profiles are explicit, separate storage:** they retain geometry—including rotation—and positive workspace placements, not names/icons/bar order. Missing or changed displays, ambiguous matches, unavailable modes, and unsafe geometry block loading; missing saved workspace IDs are skipped rather than created.
