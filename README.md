...
**See which workspaces belong to which display, arrange screens without editing configuration, and deliberately undock or redock from one compact Omarchy bar panel.**

**Plugin ID:** `display.workspaces` · **Version:** 2.6.0
...
Real plugin UI with **synthetic example displays/workspaces**, captured on an opaque, neutral background—not a personal desktop. Normal styling and translucency follow your Omarchy theme. Screenshots of older panel states have been removed rather than presenting controls that no longer match the stateful Undock/Redock workflow.
...
Choose an advertised resolution/refresh rate and valid scale, then drag displays on the map or use coordinates and relative-placement controls. Each display can also be disabled temporarily; Preview moves its workspaces to a remaining enabled display, and Revert or expiry restores the previous topology where recovery remains possible. **Preview (20s)** tries the complete draft live; **Apply** keeps it for this session.

The countdown is only for checking the live result and clicking Apply. Draft editing beforehand and saving a profile afterward have no time limit.

The top-right dock-transition button reflects live state. **Undock safely… → Confirm undock** verifies a healthy internal `eDP-*` panel, disables every external output, moves their workspaces to the laptop, and keeps the session change immediately. It then becomes **Redock safely… → Confirm redock**, which restores the complete pre-undock display geometry and workspace placement. Neither action suspends the machine or resets USB4 hardware.
...
**Moved the same monitor to another port?** A unique make/model/serial match can reuse its existing profile on the new connector. Manual Preview can now apply that draft even without an exact rule for the new connector—no Lua editing or replacement profile is required solely because the port changed. This does not extend automatic restoration to unconfigured connectors; see [connector changes and session ownership](docs/USAGE.md#new-connectors-and-session-ownership).
...
- **Separate presentation from live changes:** names/icons/order save immediately; layout, enable state and workspace moves use a session-only preview. Saving a profile is separate.
...
No extra Python packages, automatic layout learning, individual-window placement tracking, rotation editor, or HDR control panel. Native output management handles manual previews on new connectors; guarded runtime Lua monitor rules handle session-only enable-state changes. Both use the on-demand layout owner and independent rollback worker; no startup service is installed.
...
## Safety and limits
- **Enable/Disable is explicit and session-only:** at least one display remains enabled. Only displays disabled by the current session owner are offered for safe re-enable because their exact prior geometry is journaled. Hyprland can relocate windows when an output is disabled; workspace placement is guarded, but individual windows are not tracked or returned automatically.
...
- **New connectors can be arranged manually without adding monitor rules.** Native output management preserves unrelated display policy while applying geometry changes; guarded runtime Lua monitor rules apply enable-state changes. A transient session owner journals recovery and retains native geometry overrides. Automatic display changes still require supported exact connector rules. Dynamic, broad, conditional, or ambiguous declarations remain unsupported.
...
- `output_management.py`, `output_owner.py`: native geometry transport, guarded runtime enable-state ownership, and recovery journaling.
