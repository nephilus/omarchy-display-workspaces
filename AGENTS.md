...
Keep the product narrow: multi-display workspace visibility, straightforward session arrangement with advertised resolution/refresh and scale, named live-arrangement profiles with optional strong-identity reconnect restoration, immediate presentation preferences, and deliberate configuration cleanup. Output enable/disable and dock-transition controls are intentionally out of scope after unreliable connector and resume recovery. Do not add automatic layout learning, individual-window tracking, rotation editing, HDR controls, dependencies, telemetry, or persistent background services without a request.
...
- **Display/workspace changes are session-only.** Manual drafts require Apply within 20 seconds. Explicitly opted-in automatic restoration keeps only after its full 20-second health observation and authorization recheck. All paths use the independent rollback worker; never silently persist Hyprland layout rules. The plugin arranges only currently enabled outputs; it does not enable, disable, undock, or redock displays.
...
- Resume observation is shell-session-local and fail-closed: detect a sustained wall-clock gap, suppress automation, revert an interrupted Preview, release stale native geometry ownership, and require bounded stable healthy discovery before unblocking. A genuine later connection event may retry once; never poll indefinitely, reload configuration, enable outputs, reset docks, or claim to repair a failed kernel link.
