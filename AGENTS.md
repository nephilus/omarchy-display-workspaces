...
- **Display/workspace changes are session-only.** Ordinary manual drafts require Apply within 20 seconds. Confirmed safe-undock and redock actions are final after one confirmation: verify immediately, keep on success, and roll back on application or verification failure without entering a countdown. Explicitly opted-in automatic restoration keeps only after its full 20-second health observation and authorization recheck. All paths use the independent rollback worker; never silently persist Hyprland layout rules.
...
- Manual enable-state changes use guarded runtime `hl.monitor` rules, are session-only and require a healthy remaining output. Preflight the complete connected topology and native head/mode representation, journal recovery before mutation, and keep the independent worker responsible for rollback. Do not use output-management `disable_head`: Hyprland can immediately reapply exact Lua monitor rules and re-enable the output. Only outputs disabled by the current owner may be re-enabled from saved geometry. The single dock-transition control changes between confirmed Undock and Redock states. Undock must retain a healthy `eDP-*` output and evacuate every workspace from disabled externals; Redock must restore the complete journaled display geometry and surviving pre-undock workspace placements. Both verify and keep immediately; neither suspends or claims to reset a dock link. Profiles and automatic restoration do not change enabled topology.
