...
PanelWindow {
  ...
  BorderSurface {
    ...
    Item {
      ...
      width: Math.max(1, card.width - card.contentLeftInset - card.contentRightInset) / root.contentScale
      height: Math.max(1, card.height - card.contentTopInset - card.contentBottomInset) / root.contentScale
      scale: root.contentScale
      transformOrigin: Item.TopLeft
      Binding {
        target: contentHolder.Overlay.overlay
        property: "scale"
        value: root.contentScale
        when: root.backingWindowVisible && target !== null
