...
class LayoutSafetyTests(unittest.TestCase):
    ...
    def test_arrangement_rejects_disabled_or_missing_connectors(self):
        baseline = self.discover([{"id": 1, "monitor": "A"}, {"id": 2, "monitor": "B"}])
        positions = [{key: monitor[key] for key in arrangement.POSITION}
                     for monitor in baseline["monitors"]]
        disabled = copy.deepcopy(baseline["monitors"][1])
        baseline["monitors"] = baseline["monitors"][:1]
        baseline["disabledMonitors"] = [{**disabled, "disabled": True}]
        request = {"baseline": baseline, "positions": positions, "workspaces": []}
        with self.assertRaisesRegex(ValueError, "exactly one target for every enabled display"):
    ...
    def test_system_resume_releases_owner_and_reports_fresh_health(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            healthy = copy.deepcopy(self.current)
            healthy["ok"] = True
            with patch.object(arrangement, "runtime_dir", return_value=root), \
                    patch.object(arrangement.display_automation, "suppress") as suppress, \
                    patch.object(arrangement.output_owner, "release_after_resume") as release, \
                    patch.object(arrangement, "snapshot", return_value=healthy):
                result = arrangement.resume_refresh()
        self.assertTrue(result["resumeHealthy"])
        self.assertEqual(result["status"], "ready")
        suppress.assert_called_once()
        release.assert_called_once()

    def test_system_resume_requests_preview_revert_before_owner_release(self):
        token = "b" * 48
        state = {"token": token, "state": "pending", "phase": "waiting",
                 "deadline": 112, "message": "Preview", "request": "",
                 "baseline": self.current, "plan": self.plan}
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            arrangement.save(root, state)
            arrangement.atomic(root, "active.json", {"token": token})
            with patch.object(arrangement, "runtime_dir", return_value=root), \
                    patch.object(arrangement.display_automation, "suppress"), \
                    patch.object(arrangement.output_owner, "release_after_resume") as release:
                result = arrangement.resume_refresh()
            recovered = arrangement.load(root, token)
        self.assertEqual(result["status"], "reverting")
        self.assertEqual(recovered["request"], "revert")
